...
(function (Chart) {
    ...
    var Bar = /** @class */ (function (_super) {
        ...
        function Bar(element) {
            ...
            _this.legendItemHeight = 30;
            _this.legendWidth = 150;
            _this.colors = ['rgb(0, 113, 188)', 'rgb(0, 174, 239)', 'rgb(145, 0, 145)'];
            _this.xAxisHashHeight = 10;
            _this.layout = 'wiggle';
    ...
    Chart.Bar = Bar;
    var DailyBuild = /** @class */ (function (_super) {
        __extends(DailyBuild, _super);
        function DailyBuild(element) {
            var _this = _super.call(this, element) || this;
            _this.labelWidth = 140;
            _this.labelGutter = 10;
            _this.chartWidth = 800;
            _this.element = element;
            return _this;
        }
        ...
        DailyBuild.prototype.render = function (data) {
            ...
            var rects = g.selectAll('rect')
                ...
                .enter().append('rect')
                .attr('class', function (d) { return 'day ' + _this.getTextDataString(d); })
                .attr('x', function (d, i) { return _this.labelWidth + _this.labelGutter + x(d.date); })
                .attr('width', boxSize)
                .attr('height', boxSize);
            rects.append('title')
                .text(function (d) { return _this.iso8601(d.date) + ' - ' + _this.getTextDataString(d); });
            var ticks = x.ticks(d3.time.mondays, 1);
            // Date text boxes
            svg.append('g').attr('class', 'dates').selectAll('text')
                .data(ticks)
                .enter().append('text')
                .text(function (d) { return _this.iso8601(d); })
                .attr('transform', function (d, i) { return 'translate(' + (_this.labelWidth + _this.labelGutter + x(d) + 5) + ', ' + y(data.length + 1) + ')'; })
                .attr('text-anchor', 'start');
            // Vertical hashes at week boundaries
            svg.append('g').attr('class', 'hashes').selectAll('line')
                .data(ticks)
                .enter().append('line')
                .attr('x1', function (d) { return _this.labelWidth + _this.labelGutter + x(d); })
                .attr('x2', function (d) { return _this.labelWidth + _this.labelGutter + x(d); })
                .attr('y1', 0)
